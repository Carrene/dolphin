...
from sqlalchemy import Integer, ForeignKey, Unicode, JSON, select, bindparam, \
    join
...
class Envelop(OrderingMixin, PaginationMixin, FilteringMixin, ModifiedMixin,
    ...
    type_ = Field(Unicode(25))
    room_id = Field(Integer, ForeignKey('subscribable.id'))
...
class Message(Envelop):
    ...
    def iter_metadata_fields(cls):
        yield from super().iter_matadata_fields()
        yield MetadataField(
            name='isMine',
            key='is_mine',
            label='isMine',
